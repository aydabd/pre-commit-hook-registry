...
- Ownership/release health: Biome maintains the upstream hook and the `@biomejs/biome` package in the Biome organization; the reviewed release is exact and current for this admission.
...
- Cache/network: npm installation occurs only during pre-commit environment preparation through its cache boundary with lifecycle scripts disabled by the reviewed lock/install test. Hook execution uses the installed binary, is tested with network disabled, and performs no package-manager, installer, daemon, migration, upgrade, or download operation.
